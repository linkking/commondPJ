...
#import "YXU_BezierPathTool.h"

@implementation YXU_CornerRadiusTool
...
+ (UIImage *)drawAntiRoundedCornerImageWithRadius:(float)radius rectSize:(CGSize)rectSize fillColor:(UIColor *)fillColor cornerStyle:(UIRectCorner)cornerStyle {
    ...
    [YXU_CornerRadiusTool configPathWithBezierPath:&bezierPath Radius:radius rectSize:rectSize cornerStyle:cornerStyle fillColor:fillColor];
...
+ (UIImage *)drawAntiRoundedCornerWithRadius_TL:(float)radius_TL
    ...
    CGContextRef contextRef = UIGraphicsGetCurrentContext();
    UIBezierPath *bezierPath = [UIBezierPath bezierPath];
    [YXU_CornerRadiusTool configPathWithBezierPath:&bezierPath radius_TL:radius_TL radius_TR:radius_TR radius_BL:radius_BL radius_BR:radius_BR rectSize:rectSize fillColor:fillColor];
    CGContextDrawPath(contextRef, kCGPathFillStroke);
    UIImage *antiRoundedCornerImage = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return antiRoundedCornerImage;
...
+ (void)configPathWithBezierPath:(UIBezierPath **)bezier Radius:(float)radius rectSize:(CGSize)rectSize cornerStyle:(UIRectCorner)cornerStyle fillColor:(UIColor *)fillColor {
    ...
    YXU_BezierPathTool *tool = [[YXU_BezierPathTool alloc]initWithRadius:radius rectSize:rectSize fillColor:fillColor];
...
+ (void)configPathWithBezierPath:(UIBezierPath **)bezier
                       ...
                       fillColor:(UIColor *)fillColor {
    UIBezierPath *bezierPath = *bezier;
    YXU_BezierPathTool *tool = [[YXU_BezierPathTool alloc]initWithRadius_TopLeft:radius_TL radius_TopRight:radius_TR radius_BottomLeft:radius_BL radius_BottomRight:radius_BR rectSize:rectSize fillColor:fillColor];
    bezierPath = [tool configCornerBezierPath:bezierPath];
}
